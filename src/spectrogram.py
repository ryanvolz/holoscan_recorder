...
class SpectrogramOutput(holoscan.core.Operator):
    ...
    def __init__(
        ...
        **==Named Inputs==**
        ...
        output_path: os.PathLike
            Parent directory for writing output files
        data_subdir: os.PathLike
            Subdirectory of `output_path` for writing spectrogram data
        plot_subdir: os.PathLike
            Subdirectory of `output_path` for writing spectrogram plots
        num_spectra_per_output: int
            Number of spectra to combine in a single output, either a data sample or plot
        figsize: tuple[float, float]
            Figure size in inches given as a tuple of (width, height)
        ...
        self.data_path = self.output_path / data_subdir
        self.plot_path = self.output_path / plot_subdir
    ...
    def compute_one(self, spec_message):
        ...
        if self.dmd_writer is None:
            self.dmd_writer = drf.DigitalMetadataWriter(
                metadata_dir=str(self.data_path),
    ...
    def write_output(self):
        ...
        outpath = self.plot_path / datestr / fname
